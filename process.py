...
def _people_hint():
    """把常用同事姓名拼成给 LLM 的软提示：**仅用于纠正听错的名字，不是本次参会人名单**。空则空串。"""
    ppl = load_people()
    if not ppl:
        return ""
    return ("\n【人名纠错参照（不是本次参会人！）】我常合作的同事姓名：" + "、".join(ppl[:60]) +
            "。**用途仅限一条**：当本次转录里出现的某个人名明显是上面某人的近音错字时，纠正成上面的正确写法、"
            "并把该人发言并到这一个名下（不要再把错字版单列一个）。"
            "**严禁把这份名单当作本次参会人**——名单里的人这次**不一定在场**，"
            "**只有在本次转录里确实开口发言**的人才算参会人；名单里没在这次说话的人**一个都不要写进参会人**。"
            "名单外的新人照常保留，不要硬套成名单里的人。")

SEGMENT_SYS = (
    "你在把一段会议逐字转录**改写成通顺易读的书面中文整理稿**(不是摘要,信息不删)。"
    "转录是口语,充满重复、改口、说一半的残句、「那个那个」这类口水,还有少量同音听错的词。\n"
    "请:1) 按话题切分为若干段,每段前加简短小标题(## 开头);"
    "2) **把每段改写成通顺完整的句子**:去掉重复和改口、把残句补成完整句、明显听错的同音词按上下文和术语表纠正、"
    "指代含糊处在有把握时点明;3) **保留原意和全部信息点,不概括、不省略、不编造**——改的是读起来通不通,不是删内容;"
    "4) 只输出改写后的整理稿正文。")
...
MINUTES_SYS = (
    "你是会议纪要专家。下面是一场会议的完整转录(内容较准,但无说话人标注,人名可能有识别误差)。"
    "**按议题组织纪要,不要按人逐一归纳**——没有可靠的说话人信息,按人容易把内容安错人。"
    "以 Markdown 输出,首行为 `# 会议纪要`,严格按以下小节(某节确无内容写「无」,不编造):\n"
    "## TL;DR\n2-3 句:本次干了什么、定了什么、最关键的待办或风险。\n"
    "## 会中出现的人名\n开头加一句 `> 注:以下是转录里出现的人名(含被提及者),未用声纹区分,**不代表都到场或都发过言**,仅供检索参考。`。"
    "然后把本次转录中出现的人名一行列出(能判断明显是同一人的合并为一个;名单外的新人也保留)。"
    "**不要断言谁到场、谁主持、谁只是被提到**——这份只是人名索引,不是到场名单。\n"
    "## 讨论要点\n**按议题分组**,每个议题一个 `### 小标题`,其下用要点列出:该议题讨论了什么、有哪些方案/观点、结论倾向。**只讲议题内容,不要写「谁说的」**。\n"
    "## 会议决策\n明确的决定,若转录里说了缘由就附一句背景。\n"
    "## 待办事项\n表格:| 任务 | 负责人 | 截止时间 | 优先级 |。**负责人两个条件都满足才填:(1)转录里明确指派(如「X负责/X去做/X来接」);(2)这个名字精确出现在下方【人名纠错参照】名单里。**只要有一条不满足,负责人格**直接写「—」**——**不要写名单外的名字,不要加「(名单外)」「(推断)」之类批注,也不要猜。名单外的名字多半是识别错的,当没看见。\n"
...
def summarize_minutes(transcript):
    """按议题组织纪要(不按人归纳,避免无说话人信息时把内容安错人)。"""
    log("生成会议纪要...")
    return _clean_notes(_ask(MINUTES_SYS + _glossary_hint() + _people_hint(), transcript, 4000))
...
def _section_items(notes, header):
    ...
    for line in m.group(1).splitlines():
        if line.lstrip().startswith(">"):        # 跳过引用行(如「> 注:人名由语音识别推断…」免责声明)
            continue
...
def save_to_obsidian(name, notes, audio_filename, transcript_md=""):
    ...
    participants = _section_items(notes, "会中出现的人名")
