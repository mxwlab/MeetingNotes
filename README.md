...
把会议录音变成带说话人标签、可搜索的 Markdown 纪要。

录音转码、语音识别和说话人分离全部在你的 Mac 本机完成；只有识别后的文字会发送给 DeepSeek 整理。原始录音不会上传。
...
5. 等待自动下载运行环境和约 3 GB 的语音模型。安装成功后即可关闭终端窗口。
...
```text
录音 → 本机转码 / 转录 / 说话人分离 → 文字发送 DeepSeek → Markdown 纪要
```
...
FluidAudio 使用 Apache-2.0；MeetingNotes 增加了 `batch-transcribe` 命令。首次安装下载的 FFmpeg 7.1 构建启用了 GPL 组件，适用 GPL v2 或更高版本。完整声明和许可证见 [NOTICE](NOTICE) 与 `licenses/`。
